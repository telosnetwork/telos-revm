...
pub fn call<H: Host + ?Sized, SPEC: Spec>(interpreter: &mut Interpreter, host: &mut H) {
    ...
    #[cfg(feature = "telos")]
    let is_new_address = host.load_account(to).unwrap().is_empty;
    #[cfg(feature = "telos")]
    ...
    interpreter.next_action = InterpreterAction::Call {
        inputs: Box::new(CallInputs {
            ...
            bytecode_address: to,
            #[cfg(feature = "telos")]
            bytecode_address: if host.env().tx.revision_number == 0 {
                new_to
            } else {
                to
            },
    ...
    interpreter.next_action = InterpreterAction::Call {
        inputs: Box::new(CallInputs {
            ...
            value: if host.env().tx.revision_number == 0 {
                CallValue::Transfer(interpreter.contract.call_value)
            } else {
                CallValue::Apparent(interpreter.contract.call_value)
            },
